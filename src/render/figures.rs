...
//! figures of one mesh are a single draw.
...
// SAFETY: 4-byte scalars, 84 bytes, no padding.
...
impl Figures {
    pub(super) fn new(gpu: &Gpu, format: wgpu::TextureFormat, globals: &wgpu::BindGroupLayout) -> Self {
        ...
        let instance = wgpu::vertex_attr_array![5 => Float32x4, 6 => Float32x4, 7 => Float32x4, 8 => Float32x4, 9 => Float32x4, 10 => Uint32];
    ...
    pub(super) fn upload(&mut self, gpu: &Gpu) {
        if !self.staged.is_empty() {
            self.vertices = Some(gpu.device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("figure vertices"), contents: slice_as_bytes(&self.staged), usage: wgpu::BufferUsages::VERTEX }));
        }
    }
    ...
    pub(super) fn prepare(&mut self, gpu: &Gpu) {
        ...
        frame.retain(|d| self.meshes.get(d.mesh.0).is_some());
        if frame.is_empty() {
            return;
        }
        frame.sort_by_key(|d| d.mesh.0);
        let mut joints: Vec<[[f32; 4]; 4]> = Vec::new();
        let mut instances = Vec::with_capacity(frame.len());
        for (i, d) in frame.iter().enumerate() {
            let first = joints.len() as u32;
            joints.extend(d.joints.iter().map(Mat4::to_gpu));
            instances.push(Instance { model: d.model.to_gpu(), look: [d.fog, d.tint[0], d.tint[1], d.tint[2]], first });
            let range = self.meshes[d.mesh.0];
        ...
        if self.joints.as_ref().is_none_or(|(_, cap)| *cap < joints.len()) {
            ...
            let buffer = device.create_buffer(&wgpu::BufferDescriptor { label: Some("figure joints"), size: (cap * 64) as u64, usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
            self.bind = Some(device.create_bind_group(&wgpu::BindGroupDescriptor { label: Some("figure joints"), layout: &self.layout, entries: &[wgpu::BindGroupEntry { binding: 0, resource: buffer.as_entire_binding() }] }));
        ...
        if self.instances.as_ref().is_none_or(|(_, cap)| *cap < instances.len()) {
            ...
            let buffer = device.create_buffer(&wgpu::BufferDescriptor { label: Some("figure instances"), size: (cap * std::mem::size_of::<Instance>()) as u64, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
