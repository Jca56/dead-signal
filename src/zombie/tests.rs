...
use super::brain::{HEARING, HP, Senses, State, Zombie};
...
fn run(z: &mut Zombie, body: &mut Body, solids: &Solids, nav: Option<&NavGrid>, player: Option<Vec3>, noises: &[Vec3], seconds: f64) -> usize {
    ...
    for i in 0..steps {
        ...
        let senses = Senses { solids, nav, player, noises: heard };
...
#[test]
fn four_to_the_body_or_two_to_the_head() {
    let from = Vec3::ZERO;
    let mut z = Zombie::new(0.0, 1);
    for _ in 0..3 {
        assert!(!z.hurt(25.0, false, false, from));
    }
    assert!(z.hurt(25.0, false, false, from) && z.dead(), "four body shots");
    let mut z = Zombie::new(0.0, 1);
    assert!(!z.hurt(25.0, true, false, from));
    assert!(matches!(z.state, State::Stagger { .. }), "a headshot staggers");
    assert!(z.hurt(25.0, true, false, from), "two to the head");
    let mut z = Zombie::new(0.0, 1);
    assert!(!z.hurt(50.0, false, true, from) && matches!(z.state, State::Stagger { .. }), "a blow staggers");
